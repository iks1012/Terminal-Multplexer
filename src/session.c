...
int session_activity[MAX_SESSIONS];
	...
	for(int i = 0; i < MAX_SESSIONS; i++) {
		if(sessions[i] == NULL) {
			...
			if((session->pid = fork()) == 0) {
				...
				int errDup = dup2(sfd, 2);
				int outDup = dup2(sfd, 1); //stdout
				int inDup = dup2(sfd, 0); //stdin
				int closeSFD = close(sfd); 
				int closeMFD = close(mfd);
				...
				// TO BE FILLED IN
				int errChecker = execvp(path, argv);
				if(errChecker < 0 || sfd < 0 || errDup < 0 || outDup < 0 || inDup < 0 || closeSFD < 0 || closeMFD < 0){
					//return ((void*)-1);
				}
...
int getActivity(int i){
	return session_activity[i];
}
/*
...
/*
 * Basically finds and sets a new foreground session
 *
 */
void session_find(SESSION *session){
	//set fg to non null session
	if(session == fg_session){
		int index = MAX_SESSIONS;
